...
void set_slice(nmatrix* mat, int dims_count, size_t* indices, int value) {
	size_t ind = 0;
	for(int i = indices[0]; i <= indices[1]; i++) {
		for(int j = indices[2]; j <= indices[3]; j++) {
			int* element_indices = (int*)calloc(mat->ndims, sizeof(int));
			element_indices[0] = i, element_indices[1] = j;
			mat->elements[get_index(mat, element_indices)] = value;
		}
	}
}
...
int main() {
	...
	set_slice(mat, dims_count, ranges, value);
